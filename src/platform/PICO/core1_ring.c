...
{
    // Producer reads its own head with a relaxed atomic for symmetry with
    // the acquire-load of tail; on M33 a plain volatile read of head would
    // also be correct (single writer), but using __atomic_* throughout
    // keeps the memory-ordering story uniform across producer/consumer.
    const uint32_t head = __atomic_load_n(&r->head, __ATOMIC_RELAXED);
...
{
    const uint32_t tail = __atomic_load_n(&r->tail, __ATOMIC_RELAXED);
